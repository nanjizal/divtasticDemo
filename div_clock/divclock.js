...
if(!samples.examples.div_clock) samples.examples.div_clock = {}
samples.examples.div_clock.DivClock = function(p) {
	if( p === $_ ) return;
	haxe.Log.trace(js.Lib.document,{ fileName : "DivClock.hx", lineNumber : 25, className : "samples.examples.div_clock.DivClock", methodName : "new"});
	...
	new samples.examples.div_clock.application.Controller(_holder);
}
samples.examples.div_clock.DivClock.__name__ = ["samples","examples","div_clock","DivClock"];
samples.examples.div_clock.DivClock.main = function() {
	new samples.examples.div_clock.DivClock();
}
samples.examples.div_clock.DivClock.prototype.__class__ = samples.examples.div_clock.DivClock;
if(!samples.examples.div_clock.views) samples.examples.div_clock.views = {}
samples.examples.div_clock.views.DivClockView = function(holder_,win_) {
	if( holder_ === $_ ) return;
	...
	this._sec = -1;
	var dGap = 10;
	var hGap = Std["int"](20 + dGap);
	var w = Std["int"](this._holder.get_width() - dGap * 2);
	var h = Std["int"](this._holder.get_height() - hGap - dGap);
	divtastic.js.Drawing.drawGradElipse(this._holder,Std["int"](dGap),hGap,w,h,15460843,16247282,1,zpartanlite.Orientation.Vertical);
	var cx = dGap + w / 2;
	var cy = hGap + h / 2;
	this.origTitle = this._win.get_title();
	this._hourHand = new divtastic.js.Divtastic();
	this._hourHand.set_x(cx);
	this._hourHand.set_y(cy);
	samples.examples.div_clock.views.DivClockView.drawHand(this._hourHand,21,Std["int"]((h / 2 - dGap) / 1.8),3355443,3355443);
	this._holder.addChild(this._hourHand);
	this._minHand = new divtastic.js.Divtastic();
	this._minHand.set_x(cx);
	this._minHand.set_y(cy);
	samples.examples.div_clock.views.DivClockView.drawHand(this._minHand,15,Std["int"](h / 2 - dGap),14054039,14054039);
	this._holder.addChild(this._minHand);
	this._secHand = new divtastic.js.Divtastic();
	this._secHand.set_x(cx);
	this._secHand.set_y(cy);
	samples.examples.div_clock.views.DivClockView.drawHand(this._secHand,8,Std["int"](h / 2 - dGap / 2),10329706,10329706);
	this._holder.addChild(this._secHand);
	var timer = new haxe.Timer(1000);
	timer.run = $closure(this,"updateRotation");
	this.updateRotation();
}
samples.examples.div_clock.views.DivClockView.__name__ = ["samples","examples","div_clock","views","DivClockView"];
samples.examples.div_clock.views.DivClockView.drawHand = function(scope,width,length,c0,c1) {
	var inside = new divtastic.js.DisplayDiv();
	inside.set_x(-width / 2);
	inside.set_y(-length);
	scope.addChild(inside);
	divtastic.js.Drawing.drawGradTriangle(inside,0,0,width,length,c0,c1,zpartanlite.Compass.North,1,zpartanlite.Orientation.Vertical);
	var smaller = Std["int"](length / 4);
	divtastic.js.Drawing.drawGradTriangle(inside,0,length,width,smaller,c0,c1,zpartanlite.Compass.South,1,zpartanlite.Orientation.Vertical);
}
samples.examples.div_clock.views.DivClockView.prototype._holder = null;
samples.examples.div_clock.views.DivClockView.prototype._win = null;
samples.examples.div_clock.views.DivClockView.prototype._hourHand = null;
samples.examples.div_clock.views.DivClockView.prototype._minHand = null;
samples.examples.div_clock.views.DivClockView.prototype._secHand = null;
samples.examples.div_clock.views.DivClockView.prototype._sec = null;
samples.examples.div_clock.views.DivClockView.prototype._min = null;
samples.examples.div_clock.views.DivClockView.prototype._hour = null;
samples.examples.div_clock.views.DivClockView.prototype.origTitle = null;
samples.examples.div_clock.views.DivClockView.prototype.updateRotation = function() {
	var currTime = Date.now();
	this._sec = currTime.getSeconds();
	var min = currTime.getMinutes();
	var hour = currTime.getHours();
	if(hour != this._hour) this._hourHand.set_rotation(360 / 12 * (hour % 12));
	if(min != this._min) this._minHand.set_rotation(360 / 60 * min);
	this._secHand.set_rotation(360 / 60 * this._sec);
	this._min = min;
	this._hour = hour;
	this._win.set_title(this.origTitle + "  - " + this._hour + ":" + this._min + ":" + this._sec);
}
samples.examples.div_clock.views.DivClockView.prototype.__class__ = samples.examples.div_clock.views.DivClockView;
if(!samples.examples.div_clock.application) samples.examples.div_clock.application = {}
samples.examples.div_clock.application.Controller = function(holder_) {
	if( holder_ === $_ ) return;
	this._holder = holder_;
	haxe.Log.trace(this._holder,{ fileName : "Controller.hx", lineNumber : 21, className : "samples.examples.div_clock.application.Controller", methodName : "new"});
	var clockWindow = new divtastic.js.Divtastic();
	this._holder.addChild(clockWindow);
	clockWindow.set_x(40);
	clockWindow.set_y(40);
	var windowView = new divtastic.js.application.views.WindowView(clockWindow,"Div Clock",49,129,320,330,"#9d9e6a");
	new samples.examples.div_clock.views.DivClockView(clockWindow,windowView);
}
samples.examples.div_clock.application.Controller.__name__ = ["samples","examples","div_clock","application","Controller"];
samples.examples.div_clock.application.Controller.prototype._holder = null;
samples.examples.div_clock.application.Controller.prototype.__class__ = samples.examples.div_clock.application.Controller;
Std = function() { }
Std.__name__ = ["Std"];
Std["is"] = function(v,t) {
...
samples.examples.div_clock.DivClock.main()